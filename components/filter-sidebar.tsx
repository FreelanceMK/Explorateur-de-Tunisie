...
export function FilterSidebar({ filters, allPlaces, filterStats, onFiltersChange, onClearFilters, activeFilterCount, onFilterTitleClick }: FilterSidebarProps) {
  ...
  return (
    <aside className="hidden md:flex w-72 flex-col border-r border-border/50 bg-card/30 fixed left-0 top-16 bottom-0 z-30">
      <div className="flex items-center justify-between p-4 border-b border-border/50 shrink-0">
        <div className="flex items-center gap-2">
          <h2 className="font-semibold text-foreground cursor-pointer select-none" onClick={handleTitleClick}>Filters</h2>
          {activeFilterCount > 0 && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {/* Governorates */}
        <Collapsible open={isGovernoratesOpen} onOpenChange={setIsGovernoratesOpen}>
          <CollapsibleTrigger className="flex w-full items-center justify-between py-2">
